...
class IntegrationBase(ABC):
    ...
    * ``multi_install_safe`` — declare the integration safe to install
    ...
    def process_template(
        ...
        7. Render ``__SPECKIT_ASSIGNED_COMMAND_BLOCK_<NAME>__`` as the generic
           global-registry command executor.
        ...
        # 7. Render global-registry command executor blocks before resolving
        #    command references because the injected block tells users how to
        #    invoke the models command when configuration is missing.
        from .dispatch_blocks import resolve_assigned_command_blocks

        content = resolve_assigned_command_blocks(content)
